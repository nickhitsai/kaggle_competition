...
root_path = '/home/fiiser/Documents/ML/workspace/planet/1080ti_area/resnet50/5_folds_20_epoch_2/'

callbackpath =  root_path + 'Resnet50_{val_loss:.4f}-{val_binary_accuracy:.4f}-{val_recall:.4f}.hdf5'
high_recall_callbackpath =  root_path + 'Resnet50_recall_{val_loss:.4f}-{val_binary_accuracy:.4f}-{val_recall:.4f}.hdf5'

csv_filepath = '/home/fiiser/Documents/ML/workspace/planet/train_v2.csv'
logger_filepath = root_path + 'Resnet50_log.csv'
image_size = 256
...
#     beta_squared = beta ** 2
#     x = (beta_squared + 1) * (precision * recall) / (beta_squared * precision + recall + K.epsilon())
#     return (-1) * K.log(x) + (1e-5) * losses.binary_crossentropy(y_true, y_pred)
    # return (-1) * K.log(x) + K.epsilon()
...
def learning_rate_scheduler(epoch_index):
    ...
    if epoch_index < 5:                         # 1-4
        lr = 3e-05
        # lr = 1e-03
    elif epoch_index < 7 and epoch_index > 4:  # 5-6
        lr = 1e-05
    elif epoch_index < 9 and epoch_index > 6:  # 7-8
        lr = 3e-06
    elif epoch_index < 11 and epoch_index > 8: # 9-10
        lr = 1e-06
...
def model_generator():
    ...
    model.compile(optimizer=opt, loss=losses.binary_crossentropy, metrics=['binary_accuracy',
                                                                            precision,
                                                                            recall])
...
def main():
    ...
    model_path = '/home/fiiser/Documents/ML/workspace/planet/1080ti_area/resnet50/warm_up_3_epoch_1e-03/Resnet50_0.1232-0.9532-0.8091.hdf5'
